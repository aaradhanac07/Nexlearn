...
def generate_flashcards(full_text: str, count: int = 5) -> list:
...
{{"flashcards": [{{"front": "question", "back": "answer max 1 sentence", "conceptTag": "topic"}}]}}

Document (first 4000 chars):
{full_text[:4000]}"""

    try:
        raw = groq_complete(prompt, model=FAST_MODEL, temperature=0.4, max_tokens=800)
